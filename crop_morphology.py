...
import os 
from adhar_back import address_crop
...
#     files = os.listdir('./adhar_back/')
#     k = 0
#     for path in files:
def crop_address_image(path):
    if os.path.exists(path):
        #out_path = path.replace('.jpg', '.crop.png')
        path = converter_to_specified_format(path, '.jpeg')
        out_path = path.replace('.jpeg', '.crop.png')  # .png as input
        #text = pytesseract.image_to_string(Image.open(out_path), lang='eng')
        # text = pytesseract.image_to_string(Image.open(out_path), lang='eng', \
        #     config='--psm 11 --oem 3 -c tessedit_char_whitelist= 0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/')
        try:
            process_image(path, out_path)
            out = address_crop(out_path)
            return [out, out_path]
        except Exception as e:
            print('%s %s' % (path, e))
            return [' ', ' ']
    else:
        print('unable to find path in crop_morphology.py')
        return [' ', ' ']
# print("output ", crop_address_image('./adhar_back/a (1).jpeg'))